...
robomap <- function(d, area, title = NULL, caption = NULL, hovertemplate = list(flag = "", unit = "")) {
  ...
  d <- d |>
    mutate(
      leafletlabel = str_c(
        {{area}},"<br>",roboplotr:::roboplotr_format_robotable_numeric(value, flag = hovertemplate$flag)," ",{hovertemplate$unit}
      ),
  ...
  if (length(bins) == 1) {
    map_palette <-
      colorNumeric(
        getOption("roboplot.colors.traces"),
        domain = d$value,
        reverse = T
      )
  } else {
  ...
  caption <- tags$span(
    style = str_glue('opacity: 1; font-size: {getOption("roboplot.font.caption")$size}px;'),
    caption
  )
  ...
  control_style <- tags$style(
    roboplotr_set_specific_css(
      str_glue(".{robomap_id}-info"),
      "width" = "60%","background" = getOption("roboplot.colors.background"),
      "opacity" = "0.9", "font-size" = str_glue('{getOption("roboplot.font.main")$size}px'),
      "font-family" = getOption("roboplot.font.main")$family,
      "color" = getOption("roboplot.font.main")$color,
      "padding" = str_glue('{round(mainfontsize/3)}px {mainfontsize}px {round(mainfontsize/3)}px {round(mainfontsize/2)}px'), "border-radius" = "5px"
    ))

  if (length(bins) == 1) {
    this_map <- this_map |>
      addControl(control_style, position = "topleft") |>
      addControl(html = caption, position = "bottomright", className = str_glue("{robomap_id}-info")) |>
      addLegend(
    ...
    round_bins <- function(vals) {
      ...
      round_magnitude <- switch(num_digits,
                                '1' = 1,           # Units
                                '2' = 1,          # Tens
                                '3' = 10,         # Hundreds
                                '4' = 100,        # Thousands
                                '5' = 1000,       # Tens of thousands
        ...
        labFormat = function(type, cuts) {
          str_c(
            roboplotr_format_robotable_numeric(
              round_bins(tail(cuts,-1) - c(rep(-1, length(cuts) - 2), 0))
            ),
            " – ",
            roboplotr_format_robotable_numeric(
              round_bins(head(cuts,-1))
            )
