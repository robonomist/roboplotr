...
roboplot_set_options(
  artefacts = roboplot_set_artefacts(filepath = file.path(tempdir(), "plots"), auto = T, artefacts = "img_w"),
  caption_defaults = list(prefix = "Source: ", lineend = ".", updated = FALSE),
  height = 400,
  imgdl_wide = roboplot_set_imgdl_specs(
    x = 400, y = 600, suffix = "", format = "png"),
  imgdl_narrow = roboplot_set_imgdl_specs(
    x = 440, y = 550, suffix = "", format = "png"),
...
When 'xaxis_ceiling' is provided, any scatter plots 
with date format are given some breathing room to the upper end or the range by
rounding the last date unit to the next unit of the denomination given.
...
demonstration purposes we provided a width. Axis limits can also be controlled 
more precisely by-plot with 'plot_axes' parameter.
...
roboplot_set_artefacts() here to set some global options. We set 'auto' to true
(to make roboplotr() create the files automatically), and give 'filepath' where
...
roboplot_set_artefacts() to set "img_w" as the default file and specify it 
separately when something else is needed.

We also used 'verbose' as "Warning" that quiets most of the messages roboplotr
gives.
...
d <- data_get("oecd/EO") |>
  ...
  mutate(
    Country = recode(Country, "Euro area (17 countries)" = "Euro area") |>
           fct_reorder(value)
    )
...
We also used forcats::fct_rerorder to transform the column Country as a factor,
as roboplotr uses the factor levels to order the legend, and draws the traces in
that order. We could have skipped this, in which case the factoring would be 
handled by roboplot() in order of appearance.
...
Next we will create another plot.
...
roboplot_set_caption() to override the prefix instead of just giving a single 
string.
...
d |> roboplot(
  ...
  plot_axes = roboplot_set_axes(y = "CPA"),
  legend_maxwidth = 45,
  artefacts = roboplot_set_artefacts(artefacts = "img_n")
  )
```
...
roboplot_set_axes() parameter 'y' to set thecolumn "CPA" from the data to be the
yaxis of the plot.roboplot_set_axes() can take further parameters (described in
detail in function documentation), but for a horizontal bar plot with otherwise
standard data, roboplot_set_axes() can handle the ticktypes etc. for the plot by
...
We also used the parameter 'artefacts' with roboplot_set_artefacts() to get an 
...
manually into English to match the plot. We also wanted this plot to be a bit
narrower.
...
bind_rows(list(
  ...
  roboplot(Tiedot,"Crude oil price","US Dollars per barrel","EIA",width = 550,
           plot_axes = roboplot_set_axes(xformat = "%Y"),
           artefacts = roboplot_set_artefacts(artefacts = "img_n")
           )
```
...
The previous two plots hava weekly and daily data and roboplotr() changes date
...
For the next plots we want to use a stronger zeroline than what the default 
...
data_get("ecb/YC",
  ...
  roboplot(
    ...
    plot_axes = roboplot_set_axes(x = "Maturity", xticktype = "numeric"),
    width = 550,
    artefacts = roboplot_set_artefacts(artefacts = "img_n")
  )
```
...
We also wanted to prepend an explanation about the ploit to the caption, which 
roboplot_set_caption() can take as a parameter.
...
list.files(str_c(tempdir(),"/plots")) |> str_subset("png$")
