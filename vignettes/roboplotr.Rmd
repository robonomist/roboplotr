...
First off, we use roboplot_set_options() to set some global options to define 
some visual and other specifications.

```{r roboplot_options}
roboplot_set_options(
  artefacts = set_artefacts(
    filepath = file.path(tempdir(), "plots"), auto = T, artefacts = "img_w"
    ),
...
give 'filepath' where the files will be located. We have already decided that 
generally we'll use wide images (the dimensions of which we specified with 
'imgdl_wide'), so we used set_artefacts() to set "img_w" as the default file 
created and specify it separately when something else is needed.
...
predefined caption formatting previously with roboplot_set_options(), we must
...
and png files match. We also used the parameter 'artefacts' with set_artefacts()
to get an image file with different dimensions than with earlier pictures. We
defined the layout for these files earlier on with roboplot_set_options().
...
d |>
  ...
  roboplot(Area,"Electricity prices, 28-day moving average","€/MWh","Entso-E"#,
           # plot_axes = set_axes(xformat = "%m/%Y")
           )
```
...
last 
We 
provided 'xformat' in plot_axes. The formatting used is 
{https://github.com/d3/d3-3.x-api-reference/blob/master/Time-Formatting.md}{d3 time format}. 
You can control y-axis with 'yformat' {https://github.com/d3/d3-3.x-api-reference/blob/master/Formatting.md#d3_forma}{d3 number format},
...
Note that fct_relevel2() from forcats behaves differently than fct_relevel().
...
the type roboplotr() will use for tick formatting. Note that to ensure the 
traces are drawn correctly, you have to order the data yourself before using it
it in roboplot().

For this plot we also wanted to prepend an explanation about the plot to the 
caption, which set_caption() can take as a parameter.
...
plots needed for your report, and .png files or every plot stored, ready to be
