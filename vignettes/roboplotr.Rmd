...
use in the function documentation. Datasets used are mostly internal to `roboplotr`
created for these examples, but they are all created with `robonomistClient` and 
the `tidyverse` family of packages, and the code for creating them is in the package 
vignettes.R file under data_raw, so you can easily create them yourself if you want 
to follow along.
...
to store any files created in. We also load the vignette data here. 
...
creating dynamic plots very short and simple.
...
the data itself as has been done in the examples that follow.
...
vignette_data_es_unemployment |>
  roboplot(
    Area,
    "Unemployment rate","%, seasonally adjusted","Eurostat",
    height = 440,
    width = 550
    )
